...
public class TpaCommand {
    ...
    private static int request(CommandContext<ServerCommandSource> context, Boolean toHere) throws CommandSyntaxException {
        ...
        if (findRequest(request.targetFrom, request.targetTo) != null) {
            targetFrom.sendMessage(Text.literal(StringUtil.replaceMsg(Oraben.cfg.tpaPendingMsg, List.of(player(targetTo)))), false);
            return Command.SINGLE_SUCCESS;
        }
        ...
        request.setTimeoutCallback(() -> {
            ...
            targetFrom.sendMessage(Text.literal(StringUtil.replaceMsg(Oraben.cfg.tpaTimeoutMsg, List.of(player(targetTo)))));
        });
        requests.add(request);

        targetTo.sendMessage(Text.literal(StringUtil.replaceMsg(Oraben.cfg.tpaRequestMsg, List.of(player(targetFrom))) + " | ")
                .append(clickableButton(ClickEvent.Action.RUN_COMMAND, "Accept", "/tpa accept " + targetFrom.getEntityName(), Formatting.GREEN)));

        targetFrom.sendMessage(Text.literal(StringUtil.replaceMsg(Oraben.cfg.tpaSendToMsg, List.of(player(targetTo), required(requiredLvl))))
    ...
    private static int cancel(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if (req != null) {
            removeRequest(req);
            targetFrom.sendMessage(Text.literal(StringUtil.replaceMsg(Oraben.cfg.tpaCancelledFromMsg, List.of(player(targetTo)))), false);
            targetTo.sendMessage(Text.literal(StringUtil.replaceMsg(Oraben.cfg.tpaCancelledToMsg, List.of(player(targetFrom)))), false);
        } else {
            targetFrom.sendMessage(Text.literal(StringUtil.replaceMsg(Oraben.cfg.tpaTimeoutMsg, List.of(player(targetTo)))));
        }
    ...
    private static int accept(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        Request req = findRequest(targetFrom.getUuid(), targetTo.getUuid());
        if (req == null) {
            targetTo.sendMessage(Text.literal(StringUtil.replaceMsg(Oraben.cfg.tpaTimeoutMsg, List.of(player(targetFrom)))));
            return Command.SINGLE_SUCCESS;
        }
        ...
        removeRequest(req);
        targetFrom.sendMessage(Text.literal(StringUtil.replaceMsg(Oraben.cfg.tpaAcceptedMsg, List.of(player(targetTo), required(requiredLvl)))), false);
    ...
    private static boolean canTeleport(ServerCommandSource source, ServerPlayerEntity targetFrom, ServerPlayerEntity targetTo, int requiredLvl) {
        if (Oraben.cfg.tpaLvlRequired != 0 && requiredLvl > targetFrom.experienceLevel) {
            source.sendFeedback(Text.literal(StringUtil.replaceMsg(Oraben.cfg.tpaNotEnoughLevelsMsg, List.of(required(requiredLvl)))), false);
            return false;
        }

        if (targetTo.world != targetFrom.world) {
            source.sendFeedback(Text.literal(StringUtil.replaceMsg(Oraben.cfg.tpaWrongDimension, List.of(player(targetFrom), world(targetFrom.world)))), false);
            return false;
        }
